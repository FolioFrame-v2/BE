...
public interface ActivityControllerDocs {
    ...
    @Operation(
            summary = "대외활동 목록 조회",
            description = "대외활동 목록을 조회합니다.\n\n" +
                    "- `category`: CONTEST(공모전) / HACKATHON(해커톤) / 미입력 시 전체\n" +
                    "- `sort`: LATEST(최신순, 기본값) / POPULAR(인기순, 북마크 많은 순) / MOST_VIEWED(조회순)\n" +
    ...
    ResponseEntity<ApiResponse<List<ActivityResDTO>>> getActivities(
            @Parameter(description = "카테고리 필터 (CONTEST / HACKATHON / 미입력 시 전체)")
            @RequestParam(required = false) ActivityCategory category,
            @Parameter(description = "정렬 기준 (LATEST: 최신순, POPULAR: 인기순, MOST_VIEWED: 조회순) — 기본값: LATEST")
    ...
    @Operation(
            summary = "내가 북마크한 대외활동 목록 조회",
            description = "로그인한 회원이 북마크한 대외활동 목록을 최신 북마크 순으로 조회합니다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "회원을 찾을 수 없습니다.")
    })
    ResponseEntity<ApiResponse<List<ActivityResDTO>>> getBookmarkedActivities(
