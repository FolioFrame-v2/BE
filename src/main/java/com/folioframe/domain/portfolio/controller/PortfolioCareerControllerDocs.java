...
@Tag(name = "Portfolio Career", description = "포트폴리오 경력 등록·조회·수정·삭제 API")
public interface PortfolioCareerControllerDocs {
    ...
    @Operation(
            summary = "경력 목록 조회",
            description = "포트폴리오에 등록된 경력 목록을 입사일 기준 최신순으로 조회합니다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "포트폴리오를 찾을 수 없습니다.")
    })
    ResponseEntity<ApiResponse<List<CareerResDTO>>> getList(
            @Parameter(description = "포트폴리오 ID", required = true) @PathVariable Long portfolioId
    );

    @Operation(
            summary = "경력 수정",
            description = "등록된 경력 정보를 수정합니다. 본인 포트폴리오의 경력만 수정 가능합니다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "수정 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "요청 값이 올바르지 않거나 해당 경력이 이 포트폴리오에 속하지 않습니다."),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "해당 포트폴리오에 접근 권한이 없습니다."),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "포트폴리오 또는 경력을 찾을 수 없습니다.")
    })
    ...
    @Operation(
            summary = "경력 삭제",
            description = "등록된 경력을 삭제합니다. 본인 포트폴리오의 경력만 삭제 가능합니다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "삭제 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "해당 경력이 이 포트폴리오에 속하지 않습니다."),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "해당 포트폴리오에 접근 권한이 없습니다."),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "포트폴리오 또는 경력을 찾을 수 없습니다.")
    })
